...
Vector3f phong(const Vector3f &L, // direction vector from the point on the surface towards a light source
	...
	const Vector3f &R, // direction pointing towards the viewer
	const Vector3f &diffuseColor,
	...
	const float alpha) // shininess constant
{
	...
	resColor = diffuse(L, N, diffuseColor, decay_rate);
...
Vector3f trace_help(const Vector3f &rayOrigin, const Vector3f &rayDirection, const std::vector<Sphere> &spheres, int depth) {
	...
	for (auto light : lightPositions) {
		...
		reverse_ray_norm.normalize();
		if (!isObstructed(bounce, light_direction, spheres)) {
			pixelColor += phong(light_direction, normal, reverse_ray_norm, reflection, nearest->surfaceColor, Vector3f::Ones(), lcontrib, lcontrib, 100);
		}
	}
...
void render(const std::vector<Sphere> &spheres)
{
	...
	std::ofstream ofs("./out005.ppm", std::ios::out | std::ios::binary);
	ofs << "P6\n" << width << " " << height << "\n255\n";

...
namespace NetworkController
{
    ...
    public static class Networking
    {
        ...
        public static void Make_Socket(string hostName, out Socket socket, out IPAddress ipAddress)
        {
        ...
        /// <returns></returns>
        public static Socket Connect_To_Server(NetworkAction theMessageProcessor, string hostName)
        {
            System.Diagnostics.Debug.WriteLine("connecting  to " + hostName);
            Socket clientSocket;
            IPAddress ipAddress;
            Make_Socket(hostName, out clientSocket, out ipAddress);
            SocketState ClientState = new SocketState(clientSocket, -1);
            ClientState.messageProcessor = theMessageProcessor;
            ClientState.theSocket.BeginConnect(ipAddress, DEFAULT_PORT, Connected_Callback, ClientState);
        ...
        public static void Get_Data(SocketState theClientState)
        {
            try
            {
                theClientState.theSocket.BeginReceive(theClientState.messageBuffer, 0, theClientState.messageBuffer.Length, SocketFlags.None, Receive_Callback, theClientState);
            }
        ...
        public static bool Send_Data(Socket socket, String data)
        {
            byte[] messageBytes = Encoding.UTF8.GetBytes(data);
            try
            {
                socket.BeginSend(messageBytes, 0, messageBytes.Length, SocketFlags.None, Send_Callback, socket);
                return true;
            }
            catch
            {
        ...
        /// </summary>
        /// <param name="ar"></param>
        private static void Connected_Callback(IAsyncResult ar)
        {

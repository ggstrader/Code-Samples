...
namespace SpreadsheetTemplate
{
  ...
  public partial class SpreadsheetPanel : UserControl
  {
    ...
    internal void GetFirstCoords(out int col, out int row)
    {
      drawingPanel.GetFirstCoords(out col, out row);
    }
    ...
    /// <returns></returns>
    public bool SetColor(Dictionary<string, string> cells)
    {
      return drawingPanel.SetColor(cells);
    }
    ...
    private class DrawingPanel : Panel
    {
      ...
      private Dictionary<Address, Color> _color;
      ...
      public DrawingPanel(SpreadsheetPanel ss)
      {
        ...
        _color = new Dictionary<Address, Color>();
        _ssp = ss;
      }
      ...
      public bool SetColor(Dictionary<string, string> cells)
      {
        ...
        {
          int row, col;
          NameToCoords(cell.Value, out row, out col);
          Address a = new Address(col, row);
          _color[a] = Color.Green;
          Invalidate();
        }
        return true;
      }
      ...
      protected override void OnPaint(PaintEventArgs e)
      {
        ...
        //Draws the right colors
        foreach (KeyValuePair<Address, Color> address in _color)
        {
          Color color = address.Value;
          int x = address.Key.Col - _firstColumn;
          int y = address.Key.Row - _firstRow;
          e.Graphics.FillRectangle(
                new SolidBrush(color),
                LABEL_COL_WIDTH + x * DATA_COL_WIDTH + 1,
                LABEL_ROW_HEIGHT + y * DATA_ROW_HEIGHT + 1,
                              DATA_COL_WIDTH - 2,
                              DATA_ROW_HEIGHT - 2);
        }
